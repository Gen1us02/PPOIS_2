...
class AddMechanismForm(forms.ModelForm):
        ...
        if selected_type:
            try:
                available_robots = Robot.objects.annotate(
                    limb_count=Count('mechanisms', filter=Q(mechanisms__type__name="Рука") & Q(mechanisms__type__name="Нога"))
                ).filter(limb_count__lt=2)
                if self.instance.pk and self.instance.robot_id:
                    available_robots |= Robot.objects.filter(pk=self.instance.robot_id.pk)
    ...
    def clean_robot_id(self):
        ...
        if robot and mechanism_type and (mechanism_type.name == "Рука" or mechanism_type.name == "Нога"):
            existing = robot.mechanisms.filter(type=mechanism_type).exclude(pk=self.instance.pk).count()
            if existing >= 2:
